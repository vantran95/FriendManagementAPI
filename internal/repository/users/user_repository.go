...
func (u RepositoryImpl) CreateUser(email string) (bool, error) {
...
func (u RepositoryImpl) ExistsByEmail(email string) (bool, error) {
	var id int
...
func (u RepositoryImpl) FindUserIdByEmail(email string) (int64, error) {
	var id int64
	err := u.DB.QueryRow("select id from user_management where email = $1", email).Scan(&id)
	if err != nil {
		return -1, err
...
func (u RepositoryImpl) FindEmailByIds(ids []int64) ([]string, error) {
